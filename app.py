...
with tab_search:
    ...
    with col_time:
        max_time = st.slider("Максимальное время (минуты)", min_value=5, max_value=120, value=60, step=5)
    with col_cat:
        category = st.selectbox("Категория блюда", ["Все", "завтрак", "обед", "ужин"], index=0)
    with col_diff:
        difficulty = st.selectbox("Сложность", ["Все", "легко", "средне", "сложно"], index=0)
    ...
    if st.session_state.search_clicked and ingredients_input:
        ...
        for recipe in recipes:
            ...
            if difficulty != "Все" and recipe.get("difficulty") != difficulty:
                continue
        ...
        if not top_matches:
            ...
            for item in top_matches:
                ...
                with col_name:
                    diff_emoji = {"легко": "🟢", "средне": "🟡", "сложно": "🔴"}.get(r.get("difficulty", ""), "⚪")
                    st.markdown(f"### {r['name']} — **{r['time']}** • {r.get('category', '—')} • {diff_emoji} {r.get('difficulty', '')}")
                with col_btn:
                    heart = "❤️" if is_favorite else "♡"
                    if st.button(f"{heart} Избранное", key=f"heart_{r['name']}"):
                        if is_favorite:
                ...
                with col1:
                    st.write(f"**🎯 Совпадение:** {item['common']}/{len(r['ingredients'])} ({round(item['score']*100)}%)")
                with col2:
                    st.write(f"**👥 Порций:** {r.get('servings', '—')}")
                with col3:
                    st.write(f"**⏱ Время:** {r['time']}")

                st.write("**🛒 Ингредиенты:** " + ", ".join(r["ingredients"]))

                if r.get("tips"):
                    st.info(f"💡 **Совет:** {r['tips']}")

                with st.expander("📋 Пошаговое приготовление"):
...
with tab_favs:
    ...
    if st.session_state.favorites:
        for fav in st.session_state.favorites:
            ...
            with col_fav_name:
                diff_emoji = {"легко": "🟢", "средне": "🟡", "сложно": "🔴"}.get(fav.get("difficulty", ""), "⚪")
                st.markdown(f"### {fav['name']} — **{fav['time']}** • {diff_emoji} {fav.get('difficulty', '')}")
            with col_fav_del:
            ...
            with col1:
                st.write(f"**👥 Порций:** {fav.get('servings', '—')}")
            with col2:
                st.write(f"**⏱ Время:** {fav['time']}")

            st.write("**🛒 Ингредиенты:** " + ", ".join(fav["ingredients"]))

            if fav.get("tips"):
                st.info(f"💡 **Совет:** {fav['tips']}")

            if fav.get("instructions"):
